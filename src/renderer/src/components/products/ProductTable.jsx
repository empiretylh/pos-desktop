    ...
    if (data) {
      ...
      return sorted_data.filter((item) => {
        if (
          ...
          IDToCategory(item.category).toLowerCase().includes(searchtext.toLowerCase())
        ) {
          return item
        }
      })
        ...
        <table className="w-full">
          <thead className="bg-primary sticky top-0 text-white">
            <tr>
              ...
              {isAdmin && <th className="border px-2 py-2">{t('Price5')}</th>}
                  ...
                  >
                    ...
                    <td
                      className={`border px-2 py-1 text-center ${
                        item.qty <= 0 ? 'bg-red-500 text-white' : ''
                      }`}
                    >
                      {item.qty}
                    </td>
                    <td
                      className={`border px-2 py-1 text-right ${
                        parseInt(item?.price) < parseInt(item?.cost) ? 'bg-red-500 text-white' : ''
                      }`}
                    >
                    ...
                    <td className="border px-2 py-1 font-mono text-right text-sm">
                      {fillBarcode(item.barcode)}
                    </td>
                  </tr>
                ))
