...
const SalesTable = ({
    ...
    if (data) {
      return data?.filter((item) => {
        if (
          ...
          item?.voucherNumber?.toLowerCase()?.includes(searchtext.toLowerCase())
        ) {
          return item
        }
      })
  ...
  const { cart, plusQty, minusQty, editQty, priceEdit, total, setCart, addListToCart } =
    useCartTemp()

  return (
    <div className={`w-full overflow-auto my-2 absolute h-full`}>
              ...
              filterData.map((item, index) => (
                <tr
                  ...
                  className={`cursor-pointer select-none ${
                    selectedRow?.receiptNumber == item?.receiptNumber
                ...
                >
                  ...
                  <td className="border px-2 py-1 text-right">
                    {numberWithCommas(parseInt(item.tax))}
                  </td>
                  <td className="border px-2 py-1 text-right">
                    {numberWithCommas(parseInt(item.discount))}
                  </td>
                  <td className="border px-2 py-1 text-right">
                    {numberWithCommas(parseInt(item.deliveryCharges))}
                  </td>
                  <td className="border px-2 py-1 text-right">
