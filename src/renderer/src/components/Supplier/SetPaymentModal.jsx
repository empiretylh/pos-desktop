...
import { changePrice, postCustomer, putSupplier, salesSetPayment } from '../../server/api';
...
const SetPaymentModal = ({ show, setShow, payment_data }) => {
    ...
    const SetPaymentToServer = useMutation(putSupplier, {
        onSuccess: () => {
    ...
    const handleSubmit = (e) => {
        ...
        SetPaymentToServer.mutate({
            product_id : parseInt(payment_data.id, 10),
            supplier_payment : price
        });
    }

    const shouldpaymentprice = useMemo(() => {
        if (payment_data) {
            let price =parseInt((parseInt(payment_data.cost) * parseInt(payment_data.qty))- parseInt(payment_data.supplier_payment));
            return price;
        }
    }, [payment_data])
            ...
            <div className="bg-white rounded-lg w-[350px]">
                <div className="flex justify-between items-center p-2">
                    <div className="flex-col">
                        ...
                        <p>Supplier အားပေးသည့် amount ကိုရိုက်ထည့်ရန်</p>
                    </div>
