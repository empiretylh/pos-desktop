...
import { changePrice, postCustomer, putCustomer, putSupplier } from '../../server/api';
import { useCustomerData } from '../../context/CustomerProvider';
import { useSupplierData } from '../../context/SupplierProvider';


const SupplierEditModal = ({ show, setShow, oldcustomer }) => {
    ...
    const [newSupplier, setNewSupplier] = useState({ name: oldcustomer?.name, description: oldcustomer?.description });
    ...
    useEffect(()=>{
        setNewSupplier({ name: oldcustomer?.name, description: oldcustomer?.description })
        if(show){
