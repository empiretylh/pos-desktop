...
import { LessThanProduct, useProductsData } from '../../context/ProductsDataProvider';
...
const LessThanQtyModal = ({ show, setShow, oldSalesData, customerid }) => {
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        if (editqty <= 0) {
            showNoti("Please Enter Valid Qty", 'bi bi-x-circle-fill text-red-500')
            return;
        }
        ...
        <div className={`fixed top-0 left-0 w-full h-full bg-gray-500 bg-opacity-50 flex justify-center items-center scale-0 duration-300 ${show ? 'scale-100' : ''}`}>
                        ...
                        FilterProductsData?.map((item, index) => (
                            <div key={index} className="grid grid-cols-3 gap-4 p-2 border-b-2 items-center">
                                ...
                                <div className="flex flex-col items-center">
                                    <h1 className="text-md font-bold">{t('Qty')}</h1>
                                    <h1 className="text-xl font-bold text-red-500">{item.qty}{t('item')}</h1>
            ...
            {showEditQty && <div style={{
                ...
                <div className="bg-white rounded-lg w-[400px] p-2">
                    <div className="flex justify-between items-center p-2">
                        <div className='flex flex-col'>
                            <h1 className="font-bold">Enter New Qty</h1>
...
export default LessThanQtyModal;